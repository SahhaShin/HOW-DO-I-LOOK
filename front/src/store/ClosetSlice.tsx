...
export const action = {
  // 옷 분야별 리스트 O
  getClothesListByType: createAsyncThunk(
    `ClosetSlice/getClothesListByType`,
    async (
      { clothesType, userId, pageNum }: ClothesListByTypeReq,
      thunkAPI
    ) => {
      return await axios({
        method: "get",
        url: `${process.env
          .REACT_APP_SERVER}/api/clothes/list?type=${clothesType}&userId=${userId}&page=${pageNum}`
      })
        .then(response => {
          let result = {
            type: clothesType,
  ...
  //OOTD X
  OOTDSave: createAsyncThunk(
    "ClosetSlice/OOTDSave",
    async ({ userId, order, slotIds }: saveOOTD, thunkAPI) => {
      return await axios({
        method: "post",
  ...
  // 새로운 옷 등록 O
  saveClothes: createAsyncThunk(
    "ClosetSlice/saveClothes",
    async (formdata, thunkAPI) => {
      await axios
        .post(`${process.env.REACT_APP_SERVER}/api/clothes`, formdata, {
          headers: {
  ...
  //특정 옷 정보 가져오기 O
  getClothInfo: createAsyncThunk(
    "ClosetSlice/getClothInfo",
    async (clothesId, thunkAPI) => {
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_SERVER}/api/clothes/detail/${clothesId}`
        );
  ...
  updateClothInfo: createAsyncThunk(
    "ClosetSlice/updateClothInfo",
    async (clothesId, thunkAPI) => {
      // try{
  ...
  //특정 옷 삭제하기 O
  deleteClothInfo: createAsyncThunk(
    "ClosetSlice/updateClothInfo",
    async (clothesId, thunkAPI) => {
      try {
        const response = await axios.delete(
          `${process.env.REACT_APP_SERVER}/api/clothes/${clothesId}`
        );
        // console.log("삭제완료");
      } catch (e) {
        console.log(e);
...
interface ClothesListByTypeReq {
  //response
  clothesType: string;
  userId: number;
  pageNum: number;
...
interface ClothesListByTypeRes {
  //response
  clothesId: number;
  photoLink: string;
}
