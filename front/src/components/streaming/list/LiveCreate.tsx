...
import { action, changeCreateModalOpen } from "../../../store/LiveSlice";
...
const LiveCreate = () => {
    ...
    for (let i = 0; i < objectListHash.length; i++) {
      for (let j in objectListHash[i].hashtagList) {
        setHashtags((hashtags) => [
          ...hashtags,
          objectListHash[i].hashtagList[j],
        ]);
      }
    }
  }
  ...
  async function makeNewLive() {
    ...
    console.log(newLive.liveSaveRequestDto);

    if (newLive.liveSaveRequestDto.photoSaveRequestDtoList.length > 0) {
      formdata.append(
        "liveSaveRequestDto",
        new Blob([JSON.stringify(newLive.liveSaveRequestDto)], {
  ...
  async function sendFormdata(formdata) {
    ...
    dispatch(action.addLive(formdata));

    let data = { size: 0, page: 0 };
    dispatch(action.getLiveTotalList(data));
  }
  ...
  return (
    <div className={`${liveCreateStyle.createTotal}`}>
      {/* left */}
      {uploadEnd ? (
        <StyledSlider {...settings} className={`${liveCreateStyle.leftSlider}`}>
          {imageSrcList?.map((oneSrc, index) => {
            return (
              <div className={`${liveCreateStyle.picture}`} key={index}>
                <img className={`${liveCreateStyle.img}`} src={oneSrc} />
              </div>
            );
          })}
        </StyledSlider>
      ) : (
        <div className={`${liveCreateStyle.left}`}>
          {/* 사진 등록 */}
          <div className={`${liveCreateStyle.picture}`}>
            {/* 이미지 등록 */}
            <div className={`${liveCreateStyle.imgRegist}`}>
              {imgSrc ? (
                <img className={`${liveCreateStyle.img}`} src={imgSrc} />
              ) : (
                ""
              )}
            </div>
          ...
          {/* 파일선택 */}
          <input
            className={`${liveCreateStyle.selectFile}`}
            type="file"
            accept="image/*"
            ref={inputRef}
            onChange={(e) => {
      ...
      {/* right */}
      <div className={`${liveCreateStyle.right}`}>
        {/* x표 */}
        <div className={`${liveCreateStyle.closeBtn}`}>
          <img
            src={process.env.PUBLIC_URL + `/img/live/closeBtn.png`}
            onClick={() => {
              dispatch(changeCreateModalOpen(false));
            }}
          />
        </div>
        ...
        {/* 해시태그 생성 */}
        {uploadEnd ? (
          <div className={`${liveCreateStyle.makeTag}`}>
            <div>추가된 해시태그</div>
            <div className={`${liveCreateStyle.tags}`}>
              {hashtags?.map((one, idx) => {
                return (
                  <div className={`${liveCreateStyle.oneTag}`} key={idx}>
                    #{one}
                  </div>
                );
              })}
          ...
          <div className={`${liveCreateStyle.makeTag}`}>
            ...
            <form
              onSubmit={addHashtag}
              className={`${liveCreateStyle.makeTagInput}`}
            >
              <input
                onChange={(e) => inputHashtag(e)}
                value={inputHash}
                type="text"
                placeholder="ex) 여름"
              />
        ...
        {/* 추가된 해시태그 */}
        {uploadEnd ? (
          <div className={`${liveCreateStyle.addedTag}`}>
            <div>문구 입력</div>
            <textarea
              onChange={(e) => inputStatement(e)}
              className={`${liveCreateStyle.textAreaSize}`}
              placeholder="어떤 기분을 공유하고 싶으신가요?"
            />
          </div>
        ) : (
          <div className={`${liveCreateStyle.addedTag}`}>
            ...
            <div className={`${liveCreateStyle.tags}`}>
              {hashtags?.map((one, idx) => {
                return (
                  <div className={`${liveCreateStyle.oneTag2}`} key={idx}>
                    #{one}
                  </div>
                );
              })}
        ...
        {uploadEnd ? (
          <div className={`${liveCreateStyle.rightBtns}`}>
            ...
            <button onClick={() => makeNewLive()}>업로드</button>
          </div>
        ) : (
          <div className={`${liveCreateStyle.rightBtns}`}>
            {page > 0 ? (
